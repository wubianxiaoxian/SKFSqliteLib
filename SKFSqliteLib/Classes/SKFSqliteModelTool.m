...
@implementation SKFSqliteModelTool
...
+ (BOOL)createTableWithModelClass: (Class)cls withUID: (NSString *)uid {
    ...
    // 1. 判断 表是否存在
    NSString *tableName = [SKFModelTool getTableNameWithModelClass:cls];
    BOOL isTableExists = [SKFTableTool isTableExists:tableName uid:uid];
    ...
    NSDictionary *ivarNameTypeDic = [SKFModelTool getModelIvarNameSqlTypeDic:cls];
    ...
    if (!isTableExists) {
        ...
        return [SKFSqliteTool dealSql:createTabelSQL withUID:uid];
    }


    // 表需要更新
    NSString *tmpTableName = [SKFModelTool getTempTableNameWithModelClass:cls];
    ...
    NSArray *columnNames = [SKFTableTool getTableAllColumnNames:tableName uid:uid];
    NSArray *modelColumnNames = [SKFModelTool getModelIvarNames:cls];
    ...
    return [SKFSqliteTool dealSqls:updateSqls withUID:uid];
...
+ (NSArray *)queryAllModels:(Class)cls uid: (NSString *)uid {
    ...
    NSArray *rowDicArray = [SKFSqliteTool querySql:sql withUID:uid];
...
+ (NSArray *)queryModels:(Class)cls sql: (NSString *)sql uid: (NSString *)uid {
    ...
    NSArray *array = [SKFSqliteTool querySql:sql withUID:uid];
...
+ (BOOL)deleteModel: (id)model uid: (NSString *)uid {
    NSString *tableName = [SKFModelTool getTableNameWithModelClass:[model class]];
    ...
    return [SKFSqliteTool dealSql:sql withUID:uid];
...
+ (BOOL)saveModel: (id)model uid: (NSString *)uid {
    ...
    NSArray *res = [SKFSqliteTool querySql:sql withUID:uid];

    // 字段
    NSArray *columnNames = [SKFModelTool getModelIvarNames:[model class]];
    ...
    if (res.count > 0) {
        ...
        return [SKFSqliteTool dealSql:insertSql withUID:uid];
    }
...
+ (NSArray *)parseRowDicArray: (NSArray *)rowDicArray withModelClass: (Class)cls {
    ...
    // 模型真正的字段 - > 类型
    NSDictionary *modelColumnNames = [SKFModelTool getModelIvarNameIvarTypeDic:cls];
...
+ (BOOL)isRequireUpdate:(Class)cls uid: (NSString *)uid {
    ...
    NSArray *tableColumnNames = [SKFTableTool getTableAllColumnNames:[SKFModelTool getTableNameWithModelClass:cls] uid:uid];
...
+ (NSDictionary *)naoTypeSQLRelation {
    ...
    return @{
             @(SKFSqliteModelToolNAONot) : @"not",
             @(SKFSqliteModelToolNAOAnd) : @"and",
             @(SKFSqliteModelToolNAOOr) : @"or"
             };
